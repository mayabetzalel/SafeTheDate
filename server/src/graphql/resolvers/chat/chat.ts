    ...
    Mutation: {
        chatCommand: async (parent: any,
            ...
            if (!res && res.error) {
                ...
                if (IntentsStore?.[intent]) responseMessage = IntentsStore[intent]?.responseMessage;
...
export type WitEntity = "wit$datetime:datetime" | "wit$location:location" | "wit$notable_person:notable_person";
...
function matchWitToSearchedRequests(responseMessage: WitResponse): WitIntent {
    ...
    var sortedIntents = responseMessage.intents.sort((intent1, intent2) => intent1.confidence - intent2.confidence);
...
function matchEntities(responseMessage: WitResponse): any {
    ...
    var entities = Object.values(responseMessage.entities)
        .map(entity =>
            Object.entries(EntitiesMaping[entity[0].name])
                .map(([key, value]) =>
